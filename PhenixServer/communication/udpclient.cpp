...
void ConnectToServer(SOCKET sock,char *username, char *serverip)
{
	...
	int iread = recv(sock, (char *)&usercount, sizeof(int), 0);
	if(iread<=0)
	{
		printf("Login error\n");
    exit(1);
	...
	for(int i = 0;i<usercount;i++)
	{
		...
		recv(sock, (char*)node, sizeof(stUserListNode), 0);
...
void * RecvThreadProc(void * lpParameter)
{
	...
	for(;;)
	{
		int iread = recv(PrimaryUDP, (char *)&recvbuf, sizeof(recvbuf), 0);
		if(iread<=0)
		{
			printf("recv error\n");
			continue;
		...
		case P2PMESSAGE:
			{
				...
				int iread1 = recv(PrimaryUDP, comemessage, 256, 0);
		...
		case GETALLUSER:
			{
				int usercount;
				int fromlen = sizeof(remote);
				int iread = recv(PrimaryUDP, (char *)&usercount, sizeof(int), 0);
				if(iread<=0)
				{
				...
				{
					stUserListNode *node = new stUserListNode;
					recv(PrimaryUDP, (char*)node, sizeof(stUserListNode), 0);
					ClientList.push_back(node);
					cout<<"Username:"<<node->userName<<endl;
					in_addr tmp;
					tmp.s_addr = htonl(node->ip);
...
int main(int argc, char* argv[])
{
    ...
    pthread_t tid;
    int err = pthread_create(&tid, NULL, &RecvThreadProc, NULL);
    if (err != 0)
